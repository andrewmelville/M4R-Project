...
class MeanReversion():
    ...
    def optimum_trade(self, currency_returns, signal_commods_returns, chunk_size = 30, lookback = 10):
        ...
        self.trade(currency_returns, signal_commods_returns, chunk_size = chunk_size, lookback = lookback, trade_info = 'Optimal')
    
    
    def noisy_trade(self, currency_returns, noisy_commods_returns, chunk_size = 30, lookback = 10):
        
        ## This function trades on the same (signal + noise) prices as the optimal
        ## trade strategy, but its signals are noisier due to its commodities signals
        ## obscured by random noise.
        
        self.trade(currency_returns, noisy_commods_returns, chunk_size = chunk_size, lookback = lookback, trade_info = 'Noisy')
    
    
    def trade(self, currency_returns, commods_returns, trade_info, chunk_size = 30, lookback = 10):
